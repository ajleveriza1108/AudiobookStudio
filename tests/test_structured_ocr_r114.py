...
def test_timeline_cells_are_narrated_month_by_month_not_across_rows():
    ...
    assert "January: Pepe Le Pew debuts in the Warner Brothers cartoon Odor-able Kitty." in result.text
    assert "February: Walt Disney's The Three Caballeros opens in New York." in result.text
    assert "March: Phyllis M. Daley is the first black nurse sworn-in as a United States Navy ensign." in result.text
    assert "Pepe Le Pew debuts in Walt Disney's Phyllis M. Daley" not in result.text
...
def test_layout_cache_schema_is_accepted(tmp_path, monkeypatch):
    ...
    page.insert_text((72, 72), "January: A correct timeline entry with enough readable words.")
    document.save(source)
    document.close()

    cache = tmp_path / "cache"
    cache.mkdir()
    text = "January: A correct timeline entry with enough readable words."
